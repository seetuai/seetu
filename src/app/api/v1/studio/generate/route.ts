...
export async function POST(req: NextRequest) {
  try {
    ...
    const imageSize = body.imageSize || '2K'; // Default to 2K if not specified
    ...
    const outputUrl = await generateWithGemini(
      ...
      modelImageUrls,
      modelAsset,
      imageSize // Pass quality setting (1K, 2K, 4K)
    );
...
async function generateWithGemini(
  ...
  imageSize: '1K' | '2K' | '4K' = '2K' // Quality setting
): Promise<string | null> {
  if (!genAI) {
  ...
  console.log(`[STUDIO] Generating image at ${imageSize} resolution`);
  ...
  try {
    const model = genAI.getGenerativeModel({
      ...
      generationConfig: {
        ...
        // Output at selected resolution with fixed aspect ratio
        imageConfig: {
          aspectRatio: '1:1', // Always maintain 1:1 square ratio
          imageSize: imageSize, // Only affects resolution quality (1K=1024, 2K=2048, 4K=4096)
        },
      } as any,
    });
    ...
    if (iterationFeedback && previousImageUrl) {
      // Build model consistency instruction for iterations
      let modelConsistencyInstruction = '';
      if (modelAsset && modelImageUrls && modelImageUrls.length > 0) {
        modelConsistencyInstruction = `
MODEL REFERENCE IMAGES: Included in this request are reference photos of the model.
CRITICAL - FACE CONSISTENCY: The model's face, skin tone, and features must remain EXACTLY the same as in the previous image and match the reference photos. Do NOT change the person's appearance.`;
      }
...
PRODUCT IMAGE: The first image shows the EXACT product to use.
${modelConsistencyInstruction}

CRITICAL - PRODUCT CONSISTENCY:
- The product must remain EXACTLY identical to the first image (product reference)
- Same design, same color, same fabric, same pattern, same brand, same label
- Do NOT modify the product in any way unless the user EXPLICITLY requests it
- If the user asks for lighting/background/pose changes, the PRODUCT stays the same

USER FEEDBACK - ONLY MAKE THESE SPECIFIC CHANGES:
${iterationFeedback}
...
1. ONLY apply the exact changes the user requested - nothing more, nothing less
2. PRODUCT LOCKED: Do NOT change the product unless user explicitly asks (e.g. "make it red")
3. ${modelAsset ? 'FACE LOCKED: Keep the exact same model face from the previous image' : 'Keep the same overall composition unless asked to change it'}
4. Changes typically requested: lighting, angle, background, pose - NOT the product itself
5. Maintain commercial quality and professional photography standards

Create an improved version applying ONLY the user's requested changes. Product${modelAsset ? ', face,' : ''} and unmentioned elements must stay identical.`;
    } else {
